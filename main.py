...
# --- دالة مساعدة لتحديد مسار الحفظ الصحيح على أندرويد ---
def get_file_path(filename):
    app = App.get_running_app()
    if app:
        # على أندرويد، نستخدم user_data_dir
        # على الكمبيوتر، نستخدم المجلد الحالي
        data_dir = getattr(app, 'user_data_dir', '.')
        return os.path.join(data_dir, filename)
    return filename

# --- أسماء الملفات فقط (سيتم دمج المسار لاحقاً) ---
SCORE_FILE_NAME = 'high_score.json'
ACHIEVEMENTS_FILE_NAME = 'achievements.json'
CURRENCY_FILE_NAME = 'currency.json'
...
DAILY_STREAK_REWARDS = [20, 30, 40, 60, 80, 100, 150]

# --- دوال حفظ وتحميل البيانات (تم تعديلها) ---

def load_data(filename_only, default_data):
    # نستخدم دالة المسار للحصول على المكان الصحيح
    full_path = get_file_path(filename_only)
...
def save_data(filename_only, data):
    # نستخدم دالة المسار للحصول على المكان الصحيح
    full_path = get_file_path(filename_only)
...
class ShopItem(BoxLayout):
    ...
    item_color = ListProperty([0, 0, 0, 1]) 
    display_status = StringProperty('Buy') 
...
class MenuScreen(Screen):
    ...
    daily_indicator = StringProperty("")
    
    def on_enter(self, *args):
        app = App.get_running_app()
        
        # تحميل البيانات باستخدام الأسماء الجديدة
        app.game_data = load_data(ACHIEVEMENTS_FILE_NAME, {"total_games": 0, "achievements": {}})
        app.high_score = load_data(SCORE_FILE_NAME, {"high_score": 0}).get("high_score", 0)
        app.currency_data = load_data(CURRENCY_FILE_NAME, {"coins": 0, "unlocked_themes": ["default"], "upgrades": {}, "daily_reward": {"last_claim": "", "streak_day": 0, "last_claim_date": ""}})
        app.stats_data = load_data(STATS_FILE_NAME, DEFAULT_STATS)
...
class GameScreen(Screen):
    ...
    def on_reaction_tap(self, instance):
        ...
        if reaction_time <= REACTION_TIME_WINDOW and self.current_reaction_color:
            ...
            save_data(STATS_FILE_NAME, app.stats_data)
    ...
    def end_game(self):
        ...
        if self.game_mode in ['classic', 'survival', 'reaction']:
            ...
            save_data(CURRENCY_FILE_NAME, app.currency_data)
    ...
    def process_stats_and_achievements(self):
        ...
        save_data(ACHIEVEMENTS_FILE_NAME, app.game_data)
        
        is_new_high_score = False
        if self.game_mode == 'classic' and self.clicks > app.high_score:
            app.high_score = self.clicks
            save_data(SCORE_FILE_NAME, {"high_score": app.high_score})
...
class DailyRewardsScreen(Screen):
    ...
    rewards_preview = StringProperty("")
    collect_enabled = BooleanProperty(False)
    collect_message = StringProperty("")
    today_reward_text = StringProperty("")
...
class ClickerApp(App):
    ...
    def build(self):
        # تحميل البيانات في user_data_dir تلقائياً عبر دالة load_data المعدلة
        self.high_score = load_data(SCORE_FILE_NAME, {"high_score": 0}).get("high_score", 0)
        self.game_data = load_data(ACHIEVEMENTS_FILE_NAME, {"total_games": 0, "achievements": {}})
        self.currency_data = load_data(CURRENCY_FILE_NAME, {"coins": 0, "unlocked_themes": ["default"], "upgrades": {}, "daily_reward": {"last_claim": "", "streak_day": 0, "last_claim_date": ""}})
        self.stats_data = load_data(STATS_FILE_NAME, DEFAULT_STATS)
